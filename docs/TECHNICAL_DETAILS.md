...
### Do Game Rules, Game Mode, Weather and Time Arrive Together With the Chunks?

A common impression on singleplayer: the moment the blocks around you appear, the progress bar is about done, and the game mode, weather and so on seem to show up at the same time as the chunks. The mod does not reorder anything and you are not misreading it; this is how vanilla sends the world:

1. **The progress bar measures the server, not the client.** The singleplayer bar comes from the server's `LevelLoadListener` with a fixed weighting of `10 (prepare spawn) + initial chunk count + 49 (the 7x7 chunks around the player's spawn)`, all of it **server-side** chunk loading/generation (`LOAD_INITIAL_CHUNKS`, `LOAD_PLAYER_CHUNKS`). The last segment, `LOAD_PLAYER_CHUNKS`, completes inside the configuration phase's `PrepareSpawnTask`, i.e. **before** the server sends `ClientboundLoginPacket`. When the bar reaches 100% the client still has no level and no player; login, rules and chunks follow.
2. **Rules and world info travel in one login batch.** `PlayerList.placeNewPlayer` sends, inside a single `suspendFlushing()`: `ClientboundLoginPacket` (already carrying the `reducedDebugInfo`, `doLimitedCrafting` and `immediateRespawn` rule bits plus the game mode, dimension and seed in `CommonPlayerSpawnInfo`) → difficulty → abilities → held slot → recipes → permissions → scoreboard → **the teleport** (the client's first coordinates) → player list (including the skin profile) → `sendLevelInfo` (world border, **clock/time**, spawn point, **weather**, `LEVEL_CHUNKS_LOAD_START`), then `resumeFlushing()`. They arrive in one TCP flush and are handled within the same or adjacent client ticks, far below what the eye can separate.
3. **Chunks follow immediately.** After `LEVEL_CHUNKS_LOAD_START`, `PlayerChunkSender` starts at nine chunks per tick from the ready chunks around the player (already loaded by `PrepareSpawnTask`), so the chunks under and around you arrive almost together with that batch; only distant chunks still need generating, batching and meshing.
4. **What this mod changes is visibility.** Vanilla shows "Loading terrain" through all of this and closes it only once the player's section is meshed, by which time everything above has long arrived. This mod drops the screen as soon as a `ClientLevel` exists and opens the gate at once, so you **see** the instant that batch lands; it creates, drops or reorders no packet (`PacketProcessorMixin` only yields a frame between two packets once handling has taken more than 8 ms, with the queue order intact).

The mental model of "loading one item after another" therefore describes the server's preparation phase, which happens before you can see the world in vanilla and in this mod alike. The only part that can visibly "appear in front of you" is the batched arrival and meshing of distant chunks after `PlayerChunkSender` starts, and at render distance 4 on a local integrated server that takes a few hundred milliseconds at most.
...
**Head and body before the client is loaded.** `LocalPlayer.tick` returns at once while `hasClientLoaded()` is false, so between login and `ServerboundPlayerLoadedPacket` (plus a 500 ms `closeDelayMs` for a newly created world) no vanilla code copies the view into `yHeadRot` or turns `yBodyRot`. Vanilla hides that window behind its screen; this mod removes the screen and grabs the mouse, so in third person the head would sit at the constructor's random yaw, interpolated from a zero `yHeadRotO` into a per-tick twitch, while the body ignores the camera. `LocalPlayerMixin` therefore runs `Player.aiStep`'s head follow and `LivingEntity.tick`'s body turn and angle normalisation (`PlaceholderVisuals.followView`) at the head of `tick`, only while not yet loaded. Rotation bookkeeping only: no movement, swing, age or packet, and the pose is continuous when vanilla takes over.
...
| `LevelExtractor` | `extract` -> `ParticleEngine.extract` | Append local break debris after vanilla's particle extraction, independent of early returns inside the engine |
| `Minecraft` | `disconnect` -> `Hud.onDisconnected` | Detach `level` before vanilla's null write when a save/KickWarn scene retains it |
...
- **Private Sodium hooks are enabled only for the exact stable build verified on each target.** Target layouts and behavior pass for `0.9.2+mc26.2` (26.2), `0.9.2+mc26.3` (Fabric 26.3), `0.9.2+mc26.1.2` (26.1.2) and `0.8.9+mc26.1.1` (26.1/26.1.1, where the terrain-shader warm-up is a no-op because those builds still compile GL programs on first use); unknown or pre-release versions safely skip those private optimizations. The 26.1.x release now shares one API-family whitelist containing both verified Sodium builds across Fabric/NeoForge; the engine and private injection code remain unchanged. Other targets retain their own policies. See [the version and runtime-verification matrix](VERSIONS.md). Installing/removing the placeholder can still rebuild renderers and stop workers, and GPU-resource teardown is not moved wholesale to another thread.
- **A real-jar headless target-transformation matrix covers common Fabric optimizers.** The combined profile includes Iris 1.11.4, ImmediatelyFast 1.16.4, Lithium 0.25.3, FerriteCore 9.0.0, EntityCulling 1.10.5, MoreCulling 1.8.1, Dynamic FPS 3.11.9, Sodium Extra 0.9.4, BadOptimizations 2.4.1, Particle Core 0.3.3, RRLS 5.2.8, Sodium 0.9.2, and ViaFabricPlus 5.0.1; additional profiles cover Bobby 5.2.15, Distant Horizons 3.2.0-b, FastQuit 3.1.5 and Reese's Sodium Options 2.2.3. The 26.x platform targets run the same kind of check through `verifyCompatibility` with their own version's jars (NeoForge 26.1.2 including ModernFix 5.27.22). This proves the NoLoadingScreen hooks survive those exact transformations, not GPU or gameplay behavior. Two conflicts found by source review were fixed on this mod's side: local break debris is appended at the `LevelExtractor` call site instead of inside `ParticleEngine.extract` (BadOptimizations returns early from an empty engine), and `Minecraft.level` is detached right after the HUD reset in `disconnect` whenever a save/KickWarn scene is about to retain the level (ModernFix scrubs the level's chunks and light engine at vanilla's null write). Except for exact Sodium-private optimizations, production compatibility uses existing chainable vanilla targets rather than fragile third-party internals.
